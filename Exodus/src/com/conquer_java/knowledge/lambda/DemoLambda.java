package com.conquer_java.knowledge.lambda;
 ...
 * <Java面向对象编程语言(万物皆对象)——思想清晰合理>——Lambda表达式(用于实现JS中的闭包功能，本质是函数式接口对象(SAM接口，Single Abstract Method Interface)，接口之中只能定义一个抽象方法)
 ...
 *  * 对于Lambda表达式λx.x/y+1来说：{ y: 1, +: [built-in addition], 1: [built-in number], a: 0, b: 1 }是环境environment，环境子集{ y: 1, +: [built-in addition], 1: [built-in number] }是闭包closure。
